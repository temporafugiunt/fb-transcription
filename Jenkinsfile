...
node 
{
  ...
  withCredentials([usernamePassword(credentialsId: 'a28fbe0d-6c96-4178-bac2-56de06cd26e2', passwordVariable: 'NPM_TOKEN', usernameVariable: 'NPM_USER')]) {
    extraDockerBuildArguments = "--build-arg ENVIRONMENT=production --build-arg NPM_AUTH_TOKEN=$env.NPM_TOKEN --build-arg NPM_USER=$env.NPM_USER --build-arg NPM_SCOPE=$env.GITHUB_USER --build-arg NPM_REGISTRY=https://npm.pkg.github.com"
